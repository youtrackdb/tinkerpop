...
public class ConnectiveStrategyTest {
    ...
    /**
     * Tests that {@link GValueManager} is properly maintaining state in cases where traversals containing a
     * {@link GValue} move within a new parent traversal.
     */
    @RunWith(Parameterized.class)
    public static class GValueMaintainedAfterMovingWithinParentTest {
        ...
        public static Iterable<Object[]> generateTestParameters() {
            return Arrays.asList(new Object[][]{
                    {__.has("name", GValue.ofString("x", "marko")).and().has("name", GValue.ofString("x", "marko")).asAdmin(), CollectionUtil.asSet("x")},
                    {__.has("name", GValue.ofString("x", "marko")).and().has("name", GValue.ofString("x", "marko")).and().has("name", GValue.ofString("x", "marko")).asAdmin(), CollectionUtil.asSet("x")},
                    {__.has("name", GValue.ofString("x", "marko")).and().has("name", GValue.ofString("y", "josh")).asAdmin(), CollectionUtil.asSet("x", "y")},
                    {__.has("name", GValue.ofString("x", "marko")).and().has("name", GValue.ofString("y", "josh")).and().has("name", GValue.ofString("z", "vadas")).asAdmin(), CollectionUtil.asSet("x", "y", "z")},
                    {__.has("name", GValue.ofString("x", "stephen")).or().where(__.out(GValue.ofString("y", "knows")).has("name", GValue.ofString("x", "stephen"))), CollectionUtil.asSet("x", "y")},
                    {__.out(GValue.ofString("w", "a")).out(GValue.ofString("x", "b")).and().out(GValue.ofString("y", "c")).or().out(GValue.ofString("z", "d")), CollectionUtil.asSet("w", "x", "y", "z")},
                    {__.as("a").out(GValue.ofString("x", "knows")).as("b").and().as("c").in(GValue.ofString("y", "likes")).as("d"), CollectionUtil.asSet("x", "y")},
                    {__.as("1").out(GValue.ofString("w", "a")).out(GValue.ofString("x", "b")).and().as("3").out(GValue.ofString("y", "c")).or().as("5").out(GValue.ofString("z", "d")), CollectionUtil.asSet("w", "x", "y", "z")},
                    {__.as("1").out(GValue.ofString("w", "a")).out(GValue.ofString("x", "b")).or().as("3").out(GValue.ofString("y", "c")).and().as("5").out(GValue.ofString("z", "d")), CollectionUtil.asSet("w", "x", "y", "z")},
            });
        }

        @Test
        public void shouldMaintainGValueManagerState() {
            // need to be sure that all variables that were added did not get lost in traversal rewriting
