...
declared scope of an enclosed step. Here, `order()` retains global scope within each child traversal. It therefore
keeps a traverser when its `by()` modulator produces no value. See <<order-step, `order()`>> for details. The following
`limit()` also operates within that child traversal.
...
`order(local)` keeps Apache TinkerPop behavior. It discards an element when its modulator produces no value. This
...
removed the element before comparison.
====
