...
When a `by()` modulator does not produce a result, it is deemed "unproductive". An unproductive modulator usually
filters the current traverser. The global `order()` step is an exception. It keeps the traverser under a `null` sort
key. The filtering behavior otherwise depends on the step.
...
in an object-local traversal. The wrapper supplies one object at a time to its child traversal. It does not change the
declared scope of an enclosed step.

Here, `order()` retains global scope within each child traversal. It therefore keeps a traverser when its `by()`
modulator produces no value. See <<order-step, `order()`>> for details. The following `limit()` also operates within
that child traversal.
...
<1> The "age" property is not <<by-step,productive>> for every vertex. Those vertices remain and sort under a `null`
key.
...
For global `order()`, a missing property key becomes `null`. It sorts first for `Order.asc` and last for `Order.desc`.
...
[NOTE, caption=YouTrackDB order semantics]
====
Global `order()` keeps a traverser when its `by()` modulator produces no value. This rule also covers a modulator that
contains a filter. The step sorts that traverser under a `null` key. The key sorts first with `Order.asc` and last with
`Order.desc`.

`order(local)` keeps Apache TinkerPop behavior. It discards a traverser when its modulator produces no value. This
filtering is the permanent default for local ordering.

The `local()` wrapper does not change the scope of an enclosed order step. Therefore, `local(order().by(k))` follows
the global retention rule. In contrast, `order(local).by(k)` follows the local filtering rule. This difference is
intentional and permanent.

`StandardOrderSemanticsStrategy` is opt in. Apply it with
`g.withStrategies(StandardOrderSemanticsStrategy.instance())`. It restores Apache TinkerPop filtering for global
`order()`.

A custom comparator on a global `by()` modulator can receive a raw `null` value. Previously, an unproductive modulator
removed the traverser before comparison.
====
