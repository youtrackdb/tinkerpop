...
declared scope of an enclosed step. Here, `order()` retains global scope within each child traversal. The following
...
====
Global `order()` keeps an element when its `by()` modulator produces no value. This rule also covers a modulator that
contains a filter. The step sorts that element under a `null` key. The key sorts first with `Order.asc` and last with
`Order.desc`.
...
`StandardOrderSemanticsStrategy` is opt in. It restores Apache TinkerPop filtering for global `order()`. The strategy
is currently a no-op for `order(local)`.
