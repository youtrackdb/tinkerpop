...
* Changed global `order()` to retain elements with unproductive `by()` modulators under a `null` sort key. Local ordering continues to filter those elements. The opt-in `StandardOrderSemanticsStrategy` restores filtering for global ordering.
