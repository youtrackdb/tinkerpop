...
* Changed global `order()` in the YouTrackDB fork to retain traversers with unproductive `by()` modulators under a `null` sort key. `local(order().by(k))` follows this rule. `order(local).by(k)` continues to filter such traversers. The opt-in `StandardOrderSemanticsStrategy` restores filtering for global ordering.
